...
export class BookShelf {
  ...
  public static readonly scrollClearsFocusedBook=true;
  ...
  public render(): HTMLElement {
    ...
    viewport.addEventListener("pointermove", (event) => { if (!this.moved&&!BookShelf.isTap(event.clientX - this.startX, event.clientY - this.startY)){this.moved = true;this.clearFocus(row,stage);} }, { passive: true });
  ...
  private handleActivation(bookId: string, row: HTMLElement,stage:HTMLElement,viewport:HTMLElement): void {
    this.updateSelection(row);this.updateLabel(bookId,stage,viewport);
  ...
  private clearFocus(row:HTMLElement,stage:HTMLElement):void{if(!this.selection.deselect())return;this.updateSelection(row);const label=stage.querySelector<HTMLElement>(".book-focus-label");if(label){label.classList.add("book-focus-label--leaving");window.setTimeout(()=>label.remove(),180);}}
}
