...
export class ReaderView extends BaseView {
  ...
  private async initialize(): Promise<void> {
    ...
    try {
      ...
      if(type!=="IMAGE_BASED"){await this.manager.saveDocumentCapability(source.book,{documentMode:type==="MIXED"?"mixed":"native",textCapability:type==="MIXED"?"partial":"full",limaCapability:type==="MIXED"?"limited":"full"});this.reflow=reflow;this.reflowBook=source.book;await this.highlights.restore(source.book.id);this.mountReflow(source.book.title);this.remeasure();this.reflow.repaginate(this.paginationMetrics());this.renderReflow();return;}await reflow.close();await this.manager.saveDocumentCapability(source.book,{documentMode:"scanned",textCapability:"none",limaCapability:"unavailable"});
