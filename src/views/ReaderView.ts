...
export class ReaderView extends BaseView {
  ...
  private renderReflowPages():void{if(!this.reflow||!this.stage)return;const host=this.stage.querySelector<HTMLElement>(".reflow-pages");if(!host)return;this.turnController?.unbind();this.desktopView?.destroy();host.replaceChildren();const current=this.reflow.currentPageNumber;
    const doublePage=this.wantsSpread();
      if(doublePage&&this.reflow.currentPage.cover){const cover=this.reflow.currentPage,next=this.reflow.pageAt(current+1),name=this.readerName?.trim(),heading=name?this.i18n.t("ui.greeting.hello",{name}):this.i18n.t("ui.greeting.readerHello");this.desktopView=new DesktopBookReaderView(cover,next,()=>void this.previous(),()=>void this.next(),paragraph=>this.renderParagraph(paragraph),{versoAfter:this.reflow.pageAt(current+2),underAfter:this.reflow.pageAt(current+3),versoBefore:null,underBefore:null},{coverState:this.desktopState.isClosed?"closed":"open",greeting:{heading,copy:this.i18n.t("ui.greeting.readerDeskCopy")},onCoverOpen:()=>this.openDesktopCover(),onCoverClose:()=>this.closeDesktopCover(),onTurnStart:direction=>this.desktopState.beginDrag(direction),onTurnSettling:direction=>this.desktopState.beginSettling(direction),onTurnFinished:()=>this.desktopState.settle()});host.append(this.desktopView.render());this.bindSelection(host);this.currentPage=current;this.totalPages=this.reflow.totalPages;this.toolbar?.update(this.currentPage,this.totalPages,this.manager.settings.settings.fontSize);this.updateCurrentChapter();this.updateProgress();return;}
  ...
  private async moveDesktop(page:number):Promise<void>{if(!this.reflow||page===this.reflow.currentPageNumber)return;this.desktopState.forceOpen();this.reflow.goTo(page);this.renderReflow();await this.saveReflow(this.reflow.currentPageNumber>=this.reflow.totalPages);}
