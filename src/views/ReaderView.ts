...
export class ReaderView extends BaseView {
  ...
  private async initialize(): Promise<void> {
    ...
    try {
      ...
      if(source.lima){this.limaDocument=source.lima;const engine=new LimaReaderEngine(),saved=source.progress?.currentLocation?.match(/^lima:([^:]+):(\d+)$/);engine.open(source.lima,this.paginationMetrics(),saved?.[1]?{blockId:saved[1],offset:Number(saved[2]??0)}:undefined,cover,!source.progress);reflow=engine;}
      else{if(source.book.fileType==="epub")throw new EpubReaderUnavailableError("Este EPUB ainda está sendo preparado. Tente novamente em instantes.");const engine=new ReflowReaderEngine(),logical=source.progress?.currentLocation?.startsWith("logical:")?Number(source.progress.currentLocation.slice(8)):0;type=await engine.open(source.blob,this.paginationMetrics(),logical,cover,!source.progress);reflow=engine;}
