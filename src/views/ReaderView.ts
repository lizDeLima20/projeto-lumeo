...
import type { ReaderCoverPage, ReaderParagraph } from "../reader/reflow/ReaderDocument";
...
import{ReaderChromeController}from"../reader/premium/ReaderChromeController";import{FocusReadingMode}from"../reader/premium/FocusReadingMode";import{ReaderProgressModel}from"../reader/premium/ReaderProgressModel";import{ReaderProgressBar}from"./ReaderProgressBar";
...
export class ReaderView extends BaseView {
  ...
  private async initialize(): Promise<void> {
    ...
    try {
      ...
      const source=await this.manager.source(this.bookId);let reflow:ReflowReaderEngine|LimaReaderEngine;let type:"TEXT_BASED"|"MIXED"|"IMAGE_BASED"="TEXT_BASED";
      const cover={title:source.book.title,author:source.book.author,image:source.book.cover||undefined};
      if(source.lima){this.limaDocument=source.lima;const engine=new LimaReaderEngine(),saved=source.progress?.currentLocation?.match(/^lima:([^:]+):(\d+)$/);engine.open(source.lima,this.paginationMetrics(),saved?.[1]?{blockId:saved[1],offset:Number(saved[2]??0)}:undefined,cover);reflow=engine;}
      else{if(source.book.fileType==="epub")throw new EpubReaderUnavailableError("Este EPUB ainda está sendo preparado. Tente novamente em instantes.");const engine=new ReflowReaderEngine(),logical=source.progress?.currentLocation?.startsWith("logical:")?Number(source.progress.currentLocation.slice(8)):0;type=await engine.open(source.blob,this.paginationMetrics(),logical,cover);reflow=engine;}
      if(type!=="IMAGE_BASED"){await this.manager.saveDocumentCapability(source.book,{documentMode:type==="MIXED"?"mixed":"native",textCapability:type==="MIXED"?"partial":"full",limaCapability:type==="MIXED"?"limited":"full"});this.reflow=reflow;this.reflowBook=source.book;await this.highlights.restore(source.book.id);this.mountReflow(source.book.title);this.renderReflow();return;}await reflow.close();await this.manager.saveDocumentCapability(source.book,{documentMode:"scanned",textCapability:"none",limaCapability:"unavailable"});
  ...
  private paginationMetrics():{width:number;height:number;fontSize:number;lineHeight:number;margin:number}{const settings=this.manager.settings.settings;return{width:Math.min(this.stage?.clientWidth??window.innerWidth,settings.readingWidth+settings.margins*2),height:this.stage?.clientHeight??window.innerHeight,fontSize:settings.fontSize,lineHeight:settings.lineHeight,margin:settings.margins};}
  private async updateReflow(settings:Parameters<ReaderManager["settings"]["updateReflow"]>[0]):Promise<void>{await this.manager.settings.updateReflow(settings);if(settings.textColor&&this.element)this.element.style.setProperty("--reader-custom-ink",settings.textColor);if(!this.reflow)return;this.reflow.repaginate(this.paginationMetrics());this.applyReflowStyles();this.renderReflow();await this.saveReflow();}
  ...
  private async applyPreferences(repaginate:boolean):Promise<void>{this.manager.settings.syncPreferences();this.applyReaderSettings();this.applyReflowStyles();if(this.reflow&&repaginate)this.reflow.repaginate(this.paginationMetrics());if(this.reflow)this.renderReflow();else await this.manager.rerender();await this.saveReflow();}
  ...
  private renderCover(cover:ReaderCoverPage):HTMLElement{const frame=this.createElement("div","reader-cover-page");if(cover.image){const image=this.createElement("img")as HTMLImageElement;image.src=cover.image;image.alt=`Capa de ${cover.title}`;frame.append(image);}else{const fallback=this.createElement("div","reader-cover-page__fallback",cover.title.slice(0,2).toUpperCase());frame.append(fallback);}return frame;}
  ...
  private async createHighlight(color:HighlightColor):Promise<Highlight|null>{if(!this.selectedAnchor||!this.reflowBook)return null;const value=await this.highlights.create(this.reflowBook.id,this.selectedAnchor,color);StudyEventBus.shared.publish({bookId:this.reflowBook.id,type:"created"});this.selections.clear();this.renderReflow();return value;}
  private async createAnnotation():Promise<void>{const highlight=await this.createHighlight("yellow");if(!highlight)return;const input=this.createElement("input","input") as HTMLInputElement;input.placeholder="Por que você marcou este trecho?";this.showDialog("Adicionar nota",input,"Salvar",async()=>{if(this.reflowBook&&input.value.trim()){await this.annotations.create(this.reflowBook.id,highlight.id,input.value);StudyEventBus.shared.publish({bookId:this.reflowBook.id,type:"created"});}});}
  ...
  private readonly handleResize = (): void => { window.clearTimeout(this.resizeTimer); this.resizeTimer = window.setTimeout(() => {if(this.reflow){this.reflow.repaginate(this.paginationMetrics());this.renderReflow();}else void this.manager.rerender();}, 180); };
