...
export class SettingsView extends BaseView {
  ...
  public render(): HTMLElement {
    ...
    section.append(this.createElement("span", "eyebrow", "Preferências"), this.createElement("h1", "page-title", "Configurações"));
    const card = this.createElement("div", "settings-card"); card.append(this.createElement("h2", "section-title", "Aparência"),
      this.createElement("p", "page-subtitle", "Escolha como o Lumeo deve aparecer neste dispositivo."));
    const choices = this.createElement("div", "theme-options");
    ([['light', '☀', 'Tema claro'], ['dark', '☾', 'Tema escuro']] as const).forEach(([theme, icon, label]) => {
  ...
  private installCard(): HTMLElement {
    ...
    const title = this.createElement("h2", "section-title", "Instalar o Lumeo"), text = this.createElement("p", "page-subtitle");
    const button = this.createElement("button", "button button--primary", "Instalar app") as HTMLButtonElement; button.type = "button";
    card.append(title, text, button);
    const paint = (): void => {
      const install = this.install;
      button.hidden = !install?.available;
      if (install?.installed) text.textContent = "O Lumeo já está instalado neste dispositivo.";
      else if (install?.available) text.textContent = "Tenha o Lumeo na tela inicial, abrindo em tela cheia e funcionando sem internet.";
      else if (install?.needsManualSteps) text.textContent = "No Safari, toque em Compartilhar e depois em “Adicionar à Tela de Início”.";
      else text.textContent = "Abra o menu do navegador e escolha “Instalar app” ou “Adicionar à tela inicial”.";
    };
    button.addEventListener("click", () => { button.disabled = true; void this.install?.install().then((outcome) => {
      if (outcome === "accepted") text.textContent = "Instalando o Lumeo…"; else paint();
  ...
  private bytes(value:number):string{return value<1024*1024?`${Math.round(value/1024)} KB`:`${(value/1024/1024).toFixed(1)} MB`;}
}
