...
/**
 * Pomodoro Lumeo on the reading screen: a small timer, the break and pause dialogs, and the
 * daily-goal question after a page turn. Reading time counts only while the page is visible
 * and the reader is in a focus period.
 */
export class ReaderPomodoroView {
  ...
  public mount(parent: HTMLElement): void {
    ...
    chip.type = "button"; chip.className = "reader-pomodoro-chip"; chip.hidden = true;
    chip.addEventListener("click", () => void this.togglePause());
    this.chip = chip; parent.append(chip);
    document.addEventListener("visibilitychange", this.visibility);
    this.sync();
  }

  /** Re-reads the preferences: turning the option on or off, or new durations. */
  public sync(): void {
    ...
    if (value.pomodoroEnabled) this.start(); else { this.stop(); this.closeDialog(); }
    this.render();
  }
  ...
  private async tick(): Promise<void> {
    ...
    if (step.changed && before === "focus" && this.cycle.phase === "break") { this.dayCache = await this.tracker.pause(); this.showBreak(); }
    else if (step.changed && this.cycle.phase === "break-over") this.showBreakOver();
  ...
  private render(): void {
    if (!this.chip) return;
    const phase = this.cycle.phase, time = ReaderPomodoroView.clock(this.cycle.remaining);
    this.chip.classList.toggle("reader-pomodoro-chip--paused", phase !== "focus");
    this.chip.textContent = phase === "focus" ? `🍅 ${time}` : phase === "break" ? `☕ ${this.t("reader.pomodoro.chipBreak", { time })}` : `⏸ ${this.t("reader.pomodoro.chipPaused")}`;
    this.chip.setAttribute("aria-label", phase === "focus" ? this.t("reader.pomodoro.timer", { time }) : this.t("reader.pomodoro.resumeAction"));
  }
  ...
  private showGoal(day: Readonly<ReadingDay>): void {
    ...
    this.openDialog(this.t("reader.pomodoro.goal.title"),
      ...
      [[this.t("reader.pomodoro.goal.continue"), () => { void this.tracker.decideGoal("continue"); this.closeDialog(); }, true],
       [this.t("reader.pomodoro.goal.tomorrow", { time }), () => void this.returnTomorrow(time), false]], "reader-pomodoro-dialog--goal");
  }

  private async returnTomorrow(time: string): Promise<void> {
