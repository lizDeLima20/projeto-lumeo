...
export class GoogleDriveLibraryService {
  ...
  public constructor(private readonly clientId = import.meta.env.VITE_GOOGLE_CLIENT_ID ?? "", private readonly fetcher: typeof fetch = fetch) { this.authorization = new GoogleDriveAuthorizationProvider(clientId); }
  ...
  public prepare(): Promise<void> {
    ...
    return this.loading ??= GoogleIdentityServices.load().then(() => undefined).catch(() => { this.loading = null; throw this.error(); });
  }
  /** Called by the explicit Connect button; reuses the one GIS OAuth2 client. */
  public async connect(): Promise<void> { try { this.token = await this.authorization.accessToken(); this.expires = Date.now() + 55 * 60_000; } catch { throw this.error(); } }
  public dispose(): void { this.token = ""; this.expires = 0; this.authorization.clear(); }
