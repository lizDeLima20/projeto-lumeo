...
export class LocalBookFileStore {
  public constructor(private readonly fallback: BookFileRepository) {}
  ...
  private async root(): Promise<FileSystemDirectoryHandle | null> { if (!this.supportsOpfs) return null; const storage = navigator.storage as StorageManager & { getDirectory(): Promise<FileSystemDirectoryHandle> }, origin = await storage.getDirectory(); return origin.getDirectoryHandle("lumeo-books", { create: true }); }
