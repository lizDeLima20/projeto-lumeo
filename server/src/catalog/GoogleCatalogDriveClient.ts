...
export class GoogleCatalogDriveClient {
  ...
  private async visitFolder(folderId: string, visited: Set<string>, books: Map<string, CatalogDriveFile>, audit: CatalogSourceAudit): Promise<void> {
    ...
    do {
      ...
      const response = await this.authorized(url); if (!response.ok) throw this.driveError(response.status); const data = await response.json() as { files?: DriveFile[]; nextPageToken?: string }; const files = data.files ?? [];
  ...
  private async accessToken(): Promise<string> { if (this.token && this.token.expiresAt > Date.now() + 60_000) return this.token.value; const now = Math.floor(Date.now() / 1000), header = this.encode({ alg: "RS256", typ: "JWT" }), payload = this.encode({ iss: this.credentials.client_email, scope: "https://www.googleapis.com/auth/drive.readonly", aud: this.credentials.token_uri ?? "https://oauth2.googleapis.com/token", iat: now, exp: now + 3600 }); const signer = createSign("RSA-SHA256"); signer.update(`${header}.${payload}`); signer.end(); const body = new URLSearchParams({ grant_type: "urn:ietf:params:oauth:grant-type:jwt-bearer", assertion: `${header}.${payload}.${signer.sign(this.credentials.private_key, "base64url")}` }); const response = await this.fetcher(this.credentials.token_uri ?? "https://oauth2.googleapis.com/token", { method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded" }, body }); if (!response.ok) throw new ApiError(503, "CATALOG_SOURCE_UNAVAILABLE", "Não foi possível autenticar no armazenamento do catálogo."); const data = await response.json() as { access_token?: string; expires_in?: number }; if (!data.access_token) throw new ApiError(503, "CATALOG_SOURCE_UNAVAILABLE", "Não foi possível autenticar no armazenamento do catálogo."); this.token = { value: data.access_token, expiresAt: Date.now() + Math.max(60, data.expires_in ?? 3600) * 1000 }; return this.token.value; }
  ...
  private driveError(status: number): ApiError { return status === 404 ? new ApiError(404, "CATALOG_FILE_UNAVAILABLE", "O livro não está mais disponível no catálogo.") : new ApiError(503, "CATALOG_SOURCE_UNAVAILABLE", "O armazenamento do catálogo não está disponível."); }
  ...
  private static parseCredentials(raw: string): ServiceAccount { if (!raw.trim()) throw new ApiError(503, "CATALOG_NOT_CONFIGURED", "O catálogo ainda não está configurado no servidor."); try { const decoded = raw.trim().startsWith("{") ? raw : Buffer.from(raw, "base64").toString("utf8"), value = JSON.parse(decoded) as ServiceAccount; if (!value.client_email || !value.private_key) throw new Error("missing fields"); return value; } catch { throw new ApiError(503, "CATALOG_NOT_CONFIGURED", "O catálogo ainda não está configurado no servidor."); } }
}
