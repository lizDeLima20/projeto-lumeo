...
describe("GoogleDriveApiProvider", () => {
  it("downloads a PDF through the official Drive API with an in-memory OAuth token", async () => {
    ...
    const provider = new GoogleDriveApiProvider(token(), async (input) => { calls.push(input); return calls.length === 1 ? metadata() : new Response("%PDF-1.7 bytes", { status: 200, headers: { "content-type": "application/pdf" } }); });
    const file = await provider.download(item(), () => undefined);
    assert.match(await file.text(), /^%PDF-/); assert.match(String(calls[0]), /fields=name/); assert.match(String(calls[1]), /alt=media/);
  });
  ...
  it("maps absent token configuration and denied consent without exposing credentials", async () => {
    ...
    await assert.rejects(() => new GoogleDriveApiProvider(token({ error: "access_denied" })).download(item(), () => undefined), (error: unknown) => error instanceof CatalogDirectDownloadError && error.code === "GOOGLE_DRIVE_CONSENT_DENIED");
  });
  it("maps Drive API status, capability, and unexpected HTML safely", async () => {
    const forbidden = new GoogleDriveApiProvider(token(), async () => new Response("", { status: 403 }));
    await assert.rejects(() => forbidden.download(item(), () => undefined), (error: unknown) => error instanceof CatalogDirectDownloadError && error.code === "GOOGLE_DRIVE_ACCESS_DENIED");
    const blocked = new GoogleDriveApiProvider(token(), async () => metadata({ capabilities: { canDownload: false } }));
    await assert.rejects(() => blocked.download(item(), () => undefined), (error: unknown) => error instanceof CatalogDirectDownloadError && error.code === "GOOGLE_DRIVE_CANNOT_DOWNLOAD");
