...
describe("Livro Real visual",()=>{
  ...
  it("coverOpeningUsesCoverPaletteAndImageOnly",async()=>{const css=await import("node:fs/promises").then(fs=>fs.readFile(new URL("../src/styles/reader.css",import.meta.url),"utf8"));assert.match(css,/body\.reader-mode #app-footer \{ display: none; \}/);assert.match(css,/--reader-paper-lit:var\(--reader-paper,#f6f1e7\)/);assert.match(css,/\.reader-cover-page\{[^{}]*--cover-color-top[^{}]*background:linear-gradient/s);assert.match(css,/\.reader-cover-page img\{[^{}]*width:100%[^{}]*height:100%[^{}]*object-fit:cover/s);assert.match(css,/\.open-book-layout--cover\{[^{}]*transform:perspective/s);assert.match(css,/\.open-book-page--under-cover/);});
});
